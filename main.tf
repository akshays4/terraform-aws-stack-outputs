...
locals {
  ...
  outputs = var.export_outputs ? { for k, v in var.outputs : replace(title(replace(k, "/[\\-_]/", " ")), " ", "") => { Value = (v), Export = {Name = "AWS::NoValue"} }} : { for k, v in var.outputs : replace(title(replace(k, "/[\\-_]/", " ")), " ", "") => { Value = (v), Export = { Name = (replace(title(replace(k, "/[\\-_]/", " ")), " ", ""))} } }
  # outputs = { for k, v in var.outputs : replace(title(replace(k, "/[\\-_]/", " ")), " ", "") => { Value = (v), Export = { Name = (replace(title(replace(k, "/[\\-_]/", " ")), " ", ""))} } }
